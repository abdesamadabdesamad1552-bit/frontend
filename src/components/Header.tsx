...
function AnnouncementBar() {
  return (
    <div className="bg-brand-black text-brand-white text-center text-sm py-2.5 px-4">
...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-brand-white/95 backdrop-blur-md border-b border-brand-beige-dark">
      <AnnouncementBar />
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 py-4">
        <div className="relative flex items-center gap-6">
          <button
            onClick={() => setShowCountryPicker(!showCountryPicker)}
            className="text-sm text-brand-gray cursor-pointer hover:text-brand-black transition-colors flex items-center gap-1"
          >
            {currentCountry.flag}
            <svg
              className="w-3 h-3"
              ...
              <div className="absolute top-full mt-2 right-0 bg-brand-white rounded-xl shadow-lg border border-brand-beige-dark py-2 z-20 min-w-[160px]">
                {countryList.map((c) => (
                  ...
                  >
                    <span>{c.flag}</span>
                    <span>{c.nameAr}</span>
                    <span className="text-xs text-brand-gray mr-auto">
                      {c.currencySymbol}
                    </span>
                  </button>
                ))}
        ...
        <div className="flex items-center gap-6">
          <Link
            href="/shop"
            className="text-sm font-medium text-brand-black hover:text-brand-gold transition-colors hidden sm:block"
          >
            تسوقي
          </Link>
          <button
